...
class Logger {
  logDir: string;
  label: string;
  _commonOptions: { console: 
    { level: string; handleExceptions: boolean; format: winston.Logform.Format; }; 
    file: { 
      level: string; 
      filename: string; 
   ...
   * @constructor Logger
   */
  constructor(options: { logDirPath: string; label: string; debugMode: boolean; }) {
    ...
    this._commonOptions = {
      console: {
        ...
        format: combine(
          colorize({ all: true }),
          printf(
            (msg: { timestamp: string | number | Date; label: any; level: any; message: any; }) => `[${new Date(msg.timestamp).toUTCString()}]: ${msg.label} : - ${msg.level
            }: ${msg.message}`
          )
        )
      },
